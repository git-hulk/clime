...
- `internal/`: non-exported packages:
  ...
  - `skill/` for repository-versioned AI agent skill management: repo identity/version parsing, Git transport, immutable snapshot cache (`~/.clime/cache`), transactional apply to agent targets, and the repository-keyed manifest (`~/.clime/skills.yaml`) with legacy migration.
